...
input_size = (480, 640)     # 480 *640 for ycbv, lom, ...    
# input_size = (960, 1280)    # 960*1280 for itodd
# input_size = (540, 720)     # for tless
# input_size = (540, 960)       # for ipd
# input_size = (2400, 2400)       # for ipd
# input_size = (600, 600)       # for ipd

def parse_args():
    parser = argparse.ArgumentParser(description='Browse a dataset on the full image')
    parser.add_argument('--config', default='configs/flow_refine/scflow2_dinov2b_rawbbox_lite.py', help='train config file path')
    parser.add_argument(
        '--skip-type',
        type=str,
        nargs='+',
        default=['ComputeBbox', 'Crop', 'Resize', 'Pad', 'RemapPose', 'Normalize', 'ToTensor', 'Collect', 'GetPointCloud', 'MatchMask'],
        help='skip some useless pipelines to better visualize')
    parser.add_argument(
        '--output-dir',
        default='results/camre_ready/c_jitter_refined_vis/vis_hb_02_refined',
        # default='results/vis_ipd_cam1/train_cam1_project_v2',
        # default='results/vis_ipd_cam1/project_0310',
        type=str,
    ...
    if args.show == 'project':
        ...
        if 'ref_rotations' in item:
            ref_image = imshow_projected_points(
                ...
                class_names=dataset.class_names, out_file=None, point_color = colors, show = not args.not_show,)
...
def main():
    ...
    if args.show in ['contour', 'mask']:
        vis_tool = Pytorch3dVisTool(cfg.model.renderer.mesh_dir, input_size, colors=colors, vis_mode=args.show)
    else:
        vis_tool = None
