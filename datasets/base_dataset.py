...
class InitalConcatDataset(Dataset):
...
class BaseDataset(Dataset):
    ...
    def evaluate(self,
        ...
        if 'bop' in metric:
            ...
            start = time.time()
            os.system(f'./eval_bop19.sh work_dirs/temp_result {bop_path} work_dirs/temp_result/scflow2_{dataset_name}-{tag}.csv {bop_json} >work_dirs/temp_result/test.log 2>&1')
            end = time.time()
            print_log(f'\n Evaulation Done! It takes {round(end-start, 2)}s')
            score_path = osp.join("work_dirs/eatimator_and_refiner/bop19_eval", f"scflow2_{dataset_name}-{tag}", 'scores_bop19.json')
    ...
    def format_results(self, results, save_dir, time=None):
        ...
        for i, result in enumerate(results):
            ...
            # into:
            if id in sequence_gts[sequence_path]:
                continue
