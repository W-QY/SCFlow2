...
@PIPELINES.register_module()
class ConvertRGBToGrayscale:

    def __call__(self, results):
        if 'img' in results:
            results['img'] = [self._convert_to_grayscale(img) for img in results['img']]

        # if 'rendered_images' in results:
        #     results['rendered_images'] = self._convert_rendered_to_grayscale(results['rendered_images'])
    ...
    def _convert_to_grayscale(img):
        if img.ndim != 3 or img.shape[2] != 3:
            raise ValueError("The input is not an image of dimension (H, W, 3)")
        gray = cv2.cvtColor(img, cv2.COLOR_RGB2GRAY)
        gray_3ch = cv2.merge([gray, gray, gray])
        return gray_3ch

    # @staticmethod
    # def _convert_rendered_to_grayscale(rendered_images):
    #     if rendered_images.ndim != 4 or rendered_images.shape[1] != 3:
    #         raise ValueError("rendered_images must be an array of shape (N, 3, H, W).")
    #     rendered_images = np.transpose(rendered_images, (0, 2, 3, 1))  # (N, H, W, 3)
    #     gray_images = []
    #     for img in rendered_images:
    #         gray = cv2.cvtColor(img, cv2.COLOR_RGB2GRAY)
    #         gray_3ch = cv2.merge([gray, gray, gray])
    #         gray_images.append(gray_3ch)
    #     gray_images = np.stack(gray_images, axis=0)  # (N, H, W, 3)
    #     return np.transpose(gray_images, (0, 3, 1, 2))  # (N, 3, H, W)
