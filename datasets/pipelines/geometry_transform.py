    ...
    def _resize_depths(self, results):
        ...
        for field in results.get('depth_fields', ['depths']):
            ...
            for i in range(num_patches):
                ...
                # if np.count_nonzero(resized_depth > 0) < 256:
                #     return None
...
@PIPELINES.register_module()
class DepthAug:
    def __init__(self, p_rd_block=0.2, p_aug1 = 0.2, p_aug2 = 0.2):
        self.p_rd_block = p_rd_block
        self.p_aug1 = p_aug1
        self.p_aug2 = p_aug2

    def random_block_patches(self, depth_map, max_area_cov=0.1, max_patch_nb=3):
        """
        Add random square or circular occlusion blocks to the depth map (for NumPy depth maps).

        Args:
            depth_map (np.ndarray): Input depth map, shape (H, W) or (B, H, W).
            max_area_cov (float): Maximum coverage area ratio.
            max_patch_nb (int): Maximum number of occlusion blocks.

        Returns:
            np.ndarray: Depth map after adding occlusion blocks.
        """

        if depth_map.ndim == 2:
            depth_map = depth_map[np.newaxis, ...]
        assert depth_map.ndim == 3, "Input must have shape (B, H, W) or (H, W)."
        batchsize, height, width = depth_map.shape
        def square_patch(depth_map, max_coverage=0.05):
            data_map = depth_map.copy()
            coverage = np.random.uniform(0.01, max_coverage, size=batchsize)
            patch_size = (np.sqrt(coverage) * min(height, width)).astype(int)
            square_mask = np.zeros_like(data_map, dtype=np.float32)
            for ix in range(batchsize):
                size = patch_size[ix]
                x_offset = np.random.randint(0, width - size + 1)
                y_offset = np.random.randint(0, height - size + 1)
                square_mask[ix, y_offset:y_offset+size, x_offset:x_offset+size] = 1
                data_map[ix] *= (1 - square_mask[ix])
            return data_map
        def circle_patch(depth_map, max_coverage=0.05):
            data_map = depth_map.copy()
            coverage = np.random.uniform(0.01, max_coverage, size=batchsize)
            patch_size = (np.sqrt(coverage) * min(height, width)).astype(int)
            circle_mask = np.zeros_like(data_map, dtype=np.float32)
            for ix in range(batchsize):
                size = patch_size[ix]
                radius = size / 2.0
                x_offset = np.random.randint(0, width - size + 1)
                y_offset = np.random.randint(0, height - size + 1)
                cx, cy = x_offset + radius, y_offset + radius
                y, x = np.ogrid[:height, :width]
                distance = np.sqrt((x - cx)**2 + (y - cy)**2)
                circle_mask[ix] = (distance <= radius).astype(np.float32)
                data_map[ix] *= (1 - circle_mask[ix])
            return data_map
        new_map = depth_map
        max_coverage = max_area_cov / max_patch_nb
        for _ in range(max_patch_nb):
            prob = np.random.rand()
            if prob <= 0.5:
                new_map = square_patch(new_map, max_coverage)
            else:
                new_map = circle_patch(new_map, max_coverage)
        if new_map.shape[0] == 1:
            new_map = new_map[0]
        return new_map
    ...
    def augment_depth(self, depth):
        ...
        depth_augmented = augmenters(image=depth[..., np.newaxis])
        return depth_augmented[..., 0]

    def augment_depth_plus(self, depth):
        augmenters = iaa.Sequential([
            iaa.CoarseDropout(p=(0.01, 0.05),  
                            size_percent=(0.1, 0.2),
                            ),
    ...
    def __call__(self, results):
        gt_depths = results['depths']
        aug_depths = []
        for aug_depth in gt_depths:
            if np.random.rand() < self.p_rd_block:
                aug_depth = self.random_block_patches(aug_depth)
            if np.random.rand() < self.p_aug1:
                aug_depth = self.augment_depth(aug_depth)
            if np.random.rand() < self.p_aug1:
                aug_depth = self.augment_depth_plus(aug_depth)
            aug_depths.append(aug_depth)
        results['depths'] = np.array(aug_depths)
        return results


@PIPELINES.register_module()
class DepthAug_test:
    def __init__(self, p_rd_block=0.0, p_aug1 = 0.0):
        self.p_rd_block = p_rd_block
        self.p_aug1 = p_aug1

    def random_block_patches(self, depth_map, max_area_cov=0.1, max_patch_nb=5):
        """
        Add random square or circular occlusion blocks to the depth map (for NumPy depth maps).
        ...
        def square_patch(depth_map, max_coverage=0.05):
            data_map = depth_map.copy()
            coverage = np.random.uniform(0.0001, max_coverage, size=batchsize)
            patch_size = (np.sqrt(coverage) * min(height, width)).astype(int)
            square_mask = np.zeros_like(data_map, dtype=np.float32)
            for ix in range(batchsize):
                size = patch_size[ix]
    ...
    def augment_depth(self, depth, pp):
        augmenters = iaa.Sequential([
            iaa.SaltAndPepper(p=(0.0, pp))
        ])
        depth_augmented = augmenters(image=depth[..., np.newaxis])
        return depth_augmented[..., 0]
    ...
    def __call__(self, results):
        ...
        for aug_depth in gt_depths:
            if np.random.rand() < self.p_rd_block:
                aug_depth = self.random_block_patches(aug_depth, max_area_cov=0.0005, max_patch_nb=2500)
            if np.random.rand() < self.p_aug1:
                aug_depth = self.augment_depth(aug_depth, pp=0.15)
            aug_depths.append(aug_depth)
        results['depths'] = np.array(aug_depths)
        return results
