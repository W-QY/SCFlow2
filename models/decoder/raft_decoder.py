...
# class MotionEncoderv2(BaseModule):
#     """The module of motion encoder.
...
#     _pose_channels = {'Basic': (128, 64), 'Small': (64, 32), 'Large':(128, 64)}
#     _pose_kernel = {'Basic': (7, 3), 'Small': (7, 3), 'Large':(7, 3)}
#     _pose_padding = {'Basic': (3, 1), 'Small': (3, 1), 'Large':(3, 1)}
...
#         pose_channels = self._pose_channels.get(net_type)
#         pose_kernel = self._pose_kernel.get(net_type)
#         pose_padding = self._pose_padding.get(net_type)
...
#         pose_inch = 9
#         pose_net = self._make_encoder(pose_inch, pose_channels, pose_kernel,
#                                       pose_padding, **kwargs)
#         self.pose_net = nn.Sequential(*pose_net)

#         out_inch = corr_channels[-1] + flow_channels[-1] + pose_channels[-1]
...
#     def forward(self, corr: torch.Tensor, flow: torch.Tensor, pose: torch.Tensor) -> torch.Tensor:
#         """Forward function for MotionEncoder.
...
class SceneFlowConvGRU(nn.Module):
    def __init__(self, hidden_dim=128, dilation=4):
        super(SceneFlowConvGRU, self).__init__()
...
class XHead(BaseModule):
    ...
    def __init__(self, in_channels: int, feat_channels: Sequence[int],
        ...
        if x == 'flow':
            ...
            raise ValueError(f'x must be \'flow\' \'weight\'or \'mask\', but got {x}')
