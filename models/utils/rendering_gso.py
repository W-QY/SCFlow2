...
class RendererGSO(nn.Module):
    def __init__(self,
        ...
        super(RendererGSO, self).__init__()
    ...
    def forward(self, rotations, translations, internel_k, img_metas):
        if self.training:   # train
            assert rotations.size(0) == translations.size(0) == internel_k.size(0) == len(img_metas)
            num_images = rotations.size(0)
            device = rotations.device
            meshes_to_render_list = []
            for img_meta in img_metas:
                # obj
                mesh = load_obj_mesh(img_meta['mesh_obj_paths'], device)
            ...
            assert rotations.size(0) == translations.size(0) == internel_k.size(0) == img_metas.size(0)
            num_images = rotations.size(0)
            device = rotations.device
            meshes_to_render_list = [self.meshes[label] for label in img_metas.tolist()]
